  ...
  useEffect(() => {
    const getCountry = async () => {
      const { data } = await axios.get(
        `https://wiki-countries-server.herokuapp.com/api/countries/${countryid}`
      );
      const country = await data.data[0];
        ...
        <Row>
          ...
          <Col xs={8}>
            ...
            <ListGroup className="list-group-flush">
              ...
              <ListGroupItem>
                Area: {newCountry.area} km<sup>2</sup>
              </ListGroupItem>
              <ListGroupItem>Code: {newCountry.alpha3Code}</ListGroupItem>
            </ListGroup>
            <Card.Body>
              {newCountry.borders?.map((e) => (
                <>
                  <Link to={`/${e}`}>{e}</Link> <br />
                </>
              ))}
